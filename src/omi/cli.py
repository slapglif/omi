...
def rebuild(ctx: click.Context, force: bool) -> None:
    ...
    click.echo(click.style("Rebuilding ANN index from embeddings...", fg="cyan", bold=True))
    ...
    try:
        ...
        click.echo(" → Fetching memories with embeddings...")
        cursor.execute("""
            SELECT id, embedding
            FROM memories
        ...
        if not rows:
            click.echo(click.style(" ⚠ No memories with embeddings found.", fg="yellow"))
            conn.close()
            return
        ...
        click.echo(" → Decoding embeddings...")
        embeddings_by_dim = {}

        with click.progressbar(rows, label='Processing embeddings') as bar:
            for memory_id, embedding_blob in bar:
                # Decode binary blob to list of floats
        ...
        for dim, embeddings in embeddings_by_dim.items():
            click.echo(f"\n → Rebuilding {dim}-dimensional index ({len(embeddings)} vectors)...")

            # Initialize ANN index
            ann_index = ANNIndex(str(db_path), dim=dim, enable_persistence=True)

            # Rebuild from embeddings
            ann_index.rebuild_from_embeddings(embeddings)

            # Save to disk (already done in rebuild_from_embeddings, but explicit is good)
            ann_index.save()

            click.echo(f" ✓ {dim}-dimensional index rebuilt successfully")

        click.echo()
        click.echo(click.style("✓ Index rebuild complete", fg="green", bold=True))
